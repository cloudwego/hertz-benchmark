...
func (c *Counter) Report(title string, totalns int64, concurrent int, total int64, bodySize, headerSize int) error {
	...
	var result string
	if tp999/1000 < 1 {
		result = fmt.Sprintf("[%s]: TPS: %.2f, TP99: %.2fus, TP999: %.2fus (b=%d Byte, h=%d Byte, c=%d, n=%d)",
			title, tps, tp99/1000, tp999/1000, bodySize, headerSize, concurrent, total)
	} else {
		result = fmt.Sprintf("[%s]: TPS: %.2f, TP99: %.2fms, TP999: %.2fms (b=%d Byte, h=%d Byte, c=%d, n=%d)",
			title, tps, tp99/1000000, tp999/1000000, bodySize, headerSize, concurrent, total)
	}
